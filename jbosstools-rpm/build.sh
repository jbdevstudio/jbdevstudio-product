...
usage ()
{
    ...
    echo ""
    echo "Example 1: $0 -clean -u \"http://download.jboss.org/jbosstools/neon/stable/updates/\" -mo \"--update\""
    echo ""
    echo "Example 2: $0 -clean -u \"http://download.jboss.org/jbosstools/neon/staging/updates/\" -mo \"--update\""
    echo ""
    echo "Example 3: $0 -clean -u \"http://download.jboss.org/jbosstools/neon/snapshots/updates/\" -mo \"--no-clean --update\""
...
JOB_NAME=rh-eclipse46-jbosstools
...
# remove IUs available in other rpms; depends on rh-eclipse46-jbosstools already being installed; otherwise skip this step
# Generate list of features & plugins provided by rh-eclipse46-jbosstools
...
echo ""; echo "[INFO] Build jbosstools.tar.xz ..."
time tar caf ${package_name}.tar.xz ${package_name}/
# when done (~5mins), 450M jbosstools-1.0-1.fc24.src.rpm and jbosstools.tar.xz created
...
RPM_VERSION=$(find ${yum_repo} -maxdepth 1 -type f -name "*.rpm" | grep -v ".src.rpm" | sort | head -1 | sed -e "s#.\+jbosstools-\(.\+\).rpm#\1#")
